...
provider "github" {
  token = "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"
}
...
resource "github_repository_file" "app-files" {
  ...
  repository          = github_repository.myrepo.name
  branch              = "main"
  commit_message      = "app-files added tp repo"
...
resource "aws_instance" "tf-docker-ec2" {
  ...
  user_data = <<-EOF
          ...
          TOKEN="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"
          FOLDER="https://$TOKEN@raw.githubusercontent.com/cemalsenel/bookstore-repo/main/"
          ...
          docker build -t jimmyshan/bookstoreapi:latest .
          docker-compose up -d
          EOF
  depends_on = [
    github_repository.myrepo, github_repository_file.app-files
  ]
}
